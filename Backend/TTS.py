...
async def TextToAudioFile(text) -> None:
    """This function converts the text to text to speech audio file using edge-tts."""
...
def TextToSpeech(Text, func=lambda r=None: True):
    """This function plays the text to speech audio file."""
    while True:
        try:
            asyncio.run(TextToAudioFile(Text))
            
            # Play the audio file
            playsound(r"data.mp3")
...
def TTS(Text, func=lambda r=None: True):
    """This function checks whether the text which is given to convert to speech is long enough or not and accordingly converts the text to speech."""
    Data = str(Text).split(".")
    ...
    if len(Data) > 8 and len(Text) >= 250:
        TextToSpeech(" ".join(Text.split(".")[0:2]) + ". " + random.choice(responses), func)
    else:
        TextToSpeech(Text, func)
